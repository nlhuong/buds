...
public:
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        validate_non_negative_index("weight", "Npairs", Npairs);
        context__.validate_dims("data initialization", "weight", "row_vector_d", context__.to_vec(Npairs));
        validate_non_negative_index("weight", "Npairs", Npairs);
        weight = row_vector_d(static_cast<Eigen::VectorXd::Index>(Npairs));
        vals_r__ = context__.vals_r("weight");
        pos__ = 0;
        size_t weight_i_vec_lim__ = Npairs;
        for (size_t i_vec__ = 0; i_vec__ < weight_i_vec_lim__; ++i_vec__) {
            weight[i_vec__] = vals_r__[pos__++];
        }
        context__.validate_dims("data initialization", "tau_shape1", "double", context__.to_vec());
        tau_shape1 = double(0);
        vals_r__ = context__.vals_r("tau_shape1");
        pos__ = 0;
        tau_shape1 = vals_r__[pos__++];
        context__.validate_dims("data initialization", "tau_shape2", "double", context__.to_vec());
        tau_shape2 = double(0);
        ...
        context__.validate_dims("data initialization", "gamma_rho_sq", "double", context__.to_vec());
        gamma_rho_sq = double(0);
        vals_r__ = context__.vals_r("gamma_rho_sq");
        pos__ = 0;
        gamma_rho_sq = vals_r__[pos__++];
        ...
        check_greater_or_equal(function__,"weight",weight,0);
        check_greater_or_equal(function__,"tau_shape1",tau_shape1,0);
        check_greater_or_equal(function__,"tau_shape2",tau_shape2,0);
        check_greater_or_equal(function__,"gamma_bias",gamma_bias,0);
        check_greater_or_equal(function__,"gamma_rho_sq",gamma_rho_sq,0);
        ...
        validate_non_negative_index("rel_dvec2", "Npairs", Npairs);
        rel_dvec2 = row_vector_d(static_cast<Eigen::VectorXd::Index>(Npairs));
        stan::math::fill(rel_dvec2,DUMMY_VAR__);

        try {
            for (int kdx = 1; kdx <= Npairs; ++kdx) {

                stan::math::assign(get_base1_lhs(rel_dvec2,kdx,"rel_dvec2",1), pow(get_base1(dvec,kdx,"dvec",1),2));
            }
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        if (!(context__.contains_r("meanvar")))
            throw std::runtime_error("variable meanvar missing");
        vals_r__ = context__.vals_r("meanvar");
        pos__ = 0U;
        context__.validate_dims("initialization", "meanvar", "double", context__.to_vec());
        // generate_declaration meanvar
        double meanvar(0);
        meanvar = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0,meanvar);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable meanvar: ") + e.what());
        }
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        T__ meanvar;
        (void) meanvar;  // dummy to suppress unused var warning
        if (jacobian__)
            meanvar = in__.scalar_lb_constrain(0,lp__);
        else
            meanvar = in__.scalar_lb_constrain(0);
        ...
        try {
            {
                ...
                for (int k = 1; k <= Npairs; ++k) {
                    ...
                    stan::math::assign(get_base1_lhs(delta,k,"delta",1), (bias + (rho_sq * fabs((get_base1(tau,get_base1(i_idx,k,"i_idx",1),"tau",1) - get_base1(tau,get_base1(j_idx,k,"j_idx",1),"tau",1))))));
                }
                lp_accum__.add(cauchy_log<propto__>(bias, 0, gamma_bias));
                lp_accum__.add(cauchy_log<propto__>(rho_sq, 0, gamma_rho_sq));
                lp_accum__.add(cauchy_log<propto__>(meanvar, 0, gamma_epsilon));
                for (int n = 1; n <= N; ++n) {
                ...
                for (int kdx = 1; kdx <= Npairs; ++kdx) {
                    ...
                    stan::math::assign(sig_sq, (((0.001 + meanvar) * (0.01 + get_base1(rel_dvec2,kdx,"rel_dvec2",1))) + min_sigma));
                    stan::math::assign(alpha, (pow(get_base1(delta,kdx,"delta",1),2) / sig_sq));
                    stan::math::assign(beta, (get_base1(delta,kdx,"delta",1) / sig_sq));
                    lp_accum__.add((get_base1(weight,kdx,"weight",1) * gamma_log(get_base1(dvec,kdx,"dvec",1),alpha,beta)));
                }
            }
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(e,current_statement_begin__);
    ...
    void write_array(RNG& base_rng__,
        ...
        double meanvar = in__.scalar_lb_constrain(0);
        vector<double> tau;
        size_t dim_tau_0__ = N;
        for (size_t k_0__ = 0; k_0__ < dim_tau_0__; ++k_0__) {
            tau.push_back(in__.scalar_lub_constrain(0,1));
